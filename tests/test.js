...
const runInnerRequests = async (tlsClient) => {
    ...
    return responseTimes.filter((response) => response.status === 'fulfilled').map((response) => response.value);
};
...
async function fetchCookiesAndAddCookies() {
    ...
    console.log(await tlsClient.getCookiesFromSession(tlsClient.getSession(), 'https://www.google.com/'));

    console.log(
        await tlsClient.addCookiesToSession(tlsClient.getSession(), 'https://www.google.com/', [
            {
